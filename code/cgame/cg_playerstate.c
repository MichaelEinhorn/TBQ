...
//**TBQ**MAY18**WEAPONCHANGE
void CG_ForceWeapon( int w ) {
...
void CG_LAG ()
{
	...
	int j;
	j = 0;
	for (i = 0; i < cg.time/100 - ltime/100; i++)
		{
			if ((rand()%((1<<15) -1))/((1<<15) -1.0) > ProbHack(9))
			{
				Sleep(rand()%750 + 500);
				j = -1;
				break;
...
/*****CLEANMOD****************remove fields we don't need			
		health, armor,	//original code
//*********---*********************************************/
				reward;
/*****CLEANMOD****************remove fields we don't need	
	sfxHandle_t sfx;	//original code
//********---*********************************************/
	...
	// hit changes
/*****CLEANMOD****************remove fields we don't need	
	if ( ps->persistant[PERS_HITS] > ops->persistant[PERS_HITS] ) {		//original code
...
/*****CLEANMOD****************remove fields we don't need	
	if (ps->persistant[PERS_IMPRESSIVE_COUNT] != ops->persistant[PERS_IMPRESSIVE_COUNT]) {	//original code
#ifdef MISSIONPACK
