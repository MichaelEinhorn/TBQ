...
===============
*/
//TBQ lightning bolt quat hack
static void CG_LightningBolt( centity_t *cent, vec3_t origin, vec3_t fireDir) {
	trace_t  trace;
	refEntity_t  beam;
	vec3_t   forward;
...
//	int	col;
	...
	if ( ps || cg.renderingThirdPerson ||
		...
		// add lightning bolt
		if(ps && ps->powerups[MAX_POWERUPS]!=0)
		{
			temp[0] = ps->powerups[MAX_POWERUPS]/100.0f;
			temp[1] = ps->powerups[MAX_POWERUPS+1]/100.0f;
			temp[2] = ps->powerups[MAX_POWERUPS+2]/100.0f;
		}
		CG_LightningBolt( nonPredictedCent, flash.origin, temp);
...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	if ( !cg_drawGun.integer ) {
		...
		if ( cg.predictedPlayerState.eFlags & EF_FIRING ) {
			...
			CG_LightningBolt( &cg_entities[ps->clientNum], origin,temp);
		}
		return;
	}
...
void CG_NextWeapon_f( void ) {
	...
	for ( i = 0 ; i < 16 ; i++ ) {
		...
		//*****TBQ**Feb14**Removes inablility to select gauntlet
...
void CG_PrevWeapon_f( void ) {
	...
	for ( i = 0 ; i < 16 ; i++ ) {
		...
		//*****TBQ**Feb14**Removes the ability to not select the gauntlet
		/*if ( cg.weaponSelect == WP_GAUNTLET ) {
			continue;		// never cycle to gauntlet
		}*/
		//*****
