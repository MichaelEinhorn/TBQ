...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_FIRE_WEAPON:
		...
		CG_FireWeapon( cent );
		break;
	//***TBQ**APR19**cases for french and view hack events
	case EV_FRENCH:
		if (rand()%2 == 0){
			trap_S_StartSound(NULL, es->number, CHAN_VOICE, trap_S_RegisterSound("sound/TBQ/Aller.wav", qfalse));
		} else {
			trap_S_StartSound(NULL, es->number, CHAN_VOICE, trap_S_RegisterSound("sound/TBQ/Aller2.wav", qfalse));
		}
		break;
	case EV_LAG:
		if ( es->number == cg.snap->ps.clientNum ){
