...
*/
	hackName(cg.snap->ps.stats[STAT_CURHACK], str);
	//CG_CenterPrint(str, 0, 0); //***TBQ**May17**This empty string print is necessary for game to not crash even though its not a vanilla feature - i sense a similarity to cody's grapple code ;)
	w = CG_DrawStrlen( str ) * BIGCHAR_WIDTH;
	CG_DrawBigString(320 - w / 2, 64, str, 1.0F);
	//can we please just invoke the wrath of the supernatural to fix this one. It would be easier than bugfixing
	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR ) {
		CG_DrawSpectator();
		CG_DrawCrosshair();
		CG_DrawCrosshairNames();
	...
	if ( !cg.scoreBoardShowing) {
		...
		//***TBQ***if statement for hack due to death
	if(cg.snap->ps.persistant[STAT_HACKDEATH] == 1){
		CG_DrawPic(0, 0, 640, 480, trap_R_RegisterShader("icons/bluescreen.tga"));
	//***
	}
	//***
}
