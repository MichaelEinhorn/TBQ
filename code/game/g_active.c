...
void ClientThink_real( gentity_t *ent ) {
	...
	QuatID(id);
	VectorFill(client->ps.persistant[PERS_FIREX]/((2 << 15) - 1.0f), client->ps.persistant[PERS_FIREY]/((2 << 15) - 1.0f), client->ps.persistant[PERS_FIREZ]/((2 << 15) - 1.0f), temp);
	if (abs(1 - VectorLength(temp)) > 0.1f){
		VectorCopy(forward, temp);
	}
	
	VectorDiv(forward, temp, rot);
	QuatSlrp(id, rot, 0.02f, rot);
	QuatApp(temp, rot, temp);

	client->ps.persistant[PERS_FIREX] = temp[0] * ((2 << 15) - 1);
	client->ps.persistant[PERS_FIREY] = temp[1] * ((2 << 15) - 1);
	client->ps.persistant[PERS_FIREZ] = temp[2] * ((2 << 15) - 1);
