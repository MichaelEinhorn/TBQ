...
//
//
void cycleHack(gentity_t *ent){//**TBQ**MAR8**cycleHack() method
	if (ent->client->ps.weapon == WP_HACKGUN){
		//char str[16];
		if (ent->client->ps.stats[STAT_HACKS] == 0) return;
		do{
			ent->client->ps.stats[STAT_CURHACK] = (ent->client->ps.stats[STAT_CURHACK] + 1)%32; //32 is max hacks
		}while(((1 << ent->client->ps.stats[STAT_CURHACK]) & ent->client->ps.stats[STAT_HACKS]) == 0);//yes this is a do-while loop, and yes this actually is the best way to do this, as just a while loop wouldn't do it
		//hackName(ent->client->ps.stats[STAT_CURHACK],str);
		//G_Printf("Cycle %s\n", str);
		//G_AddEvent( ent, EV_PRINTHACK, ent->client->hackIndex );
	}
}
...
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles) {
	Teleport( player, origin, angles, 0 );
}
void Teleport( gentity_t *player, vec3_t origin, vec3_t angles, int vis ) {
	gentity_t	*tent;

	// use temp events at source and destination to prevent the effect
	// from getting dropped by a second player event
	if ( vis == 0 && player->client->sess.sessionTeam != TEAM_SPECTATOR ) {
...
//***********---*********************************************/	
	  if ( other->client->ps.powerups[PW_REDFLAG] ) {		// only happens in standard CTF
