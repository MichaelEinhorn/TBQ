...
#endif
	//**TBQ**Jan17**grapple works when this is gone for unknown reasons **Jan24**edit: no bad, needs futher investigation **Feb21**edit: works but still confusing see below comment
	//jayson FEB21 entry: cody un-commented the code he thought he needed to comment out because his version had code no other version had that needed to be commented out for some reason
	// impact damage
	if (other->takedamage) {
...
void G_RunMissile( gentity_t *ent ) {
	...
	if ( tr.fraction != 1 ) {
		...
		if ( tr.surfaceFlags & SURF_NOIMPACT ) {
			/**TBQ**Jan17**removed to get grapple working
			// If grapple, reset owner
			if (ent->parent && ent->parent->client && ent->parent->client->hook == ent) {
				ent->parent->client->hook = NULL;
			}
			***/
			//**TBQ**Jan17**better reset owner grapple function
...
//=============================================================================
gentity_t *mine_Hack (gentity_t *self, vec3_t start, int hackNum) {
	gentity_t	*bolt;
	...
	bolt->parent = self;
	bolt->damage = hackNum;
	bolt->splashDamage = hackNum;
	bolt->splashRadius = 100;
	bolt->methodOfDeath = MOD_HACK;
	bolt->splashMethodOfDeath = MOD_HACK;
	bolt->clipmask = MASK_SHOT;
	bolt->target_ent = NULL;
...
gentity_t *splash_Hack (gentity_t *self, vec3_t start, int hackNum, int time) {
	gentity_t	*bolt;
